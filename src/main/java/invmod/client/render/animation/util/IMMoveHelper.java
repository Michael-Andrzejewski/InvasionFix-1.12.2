...
public class IMMoveHelper extends EntityMoveHelper {
	...
	protected MoveState doGroundMovement() {
		...
		// This value is positive is the target pos is greater than the entity's current
		// pos.
		double dY = this.posY
		...
		if ((Math.abs(dX) < 0.8D) && (Math.abs(dZ) < 0.8D) && ((dY > 0.0D) || (this.entity.isHoldingOntoLadder()))) {
			...
			if (ladderPos == null)
				ladderPos = this.getClimbFace(this.entity.getPosition().up());
		...
		if ((distanceSquared < 0.01D) && (ladderPos == null))
