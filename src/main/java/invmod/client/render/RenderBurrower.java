...
public class RenderBurrower extends Render<EntityIMBurrower> {
	...
	public void doRender(EntityIMBurrower entityBurrower, double x, double y, double z, float yaw, float partialTick) {
		...
		for (int i = 0; i < 16; i++) {
			...
			renderPos[(i + 1)].setPosX((lastPos[i].getPosX() + partialTick * (pos[i].getPosX() - lastPos[i].getPosX()) - renderManager.viewerPosX) * -7.269999980926514D);
			renderPos[(i + 1)].setPosY((lastPos[i].getPosY() + partialTick * (pos[i].getPosY() - lastPos[i].getPosY()) - renderManager.viewerPosY) * -7.269999980926514D);
			renderPos[(i + 1)].setPosZ((lastPos[i].getPosZ() + partialTick * (pos[i].getPosZ() - lastPos[i].getPosZ()) - renderManager.viewerPosZ) * 7.269999980926514D);
