...
public class mod_Invasion {
	...
	public void onServerStart(FMLServerStartingEvent event) {
		...
		if ((commandManager instanceof CommandHandler)) ((CommandHandler) commandManager).registerCommand(new InvasionCommand());
	}
	...
	{		
		...
		if (DEFAULT_NIGHT_MOB_PATTERN_1_SLOTS.length == DEFAULT_NIGHT_MOB_PATTERN_1_SLOT_WEIGHTS.length) 
		{
			for (int i = 0; i < DEFAULT_NIGHT_MOB_PATTERN_1_SLOTS.length; i++) 
			{
				...
				{
					ModLogger.logWarn("Pattern 1 slot " + (i + 1) + " in config not recognised. Proceeding as blank.");
				}
			}
		} 
		else 
	...
	public static Entity[] getNightMobSpawns1(World world) 
	{
		...
		for (int i = 0; i < numberOfMobs; i++) {
			EntityIMMob mob = getMobBuilder().createMobFromConstruct(((IEntityIMPattern) mobPool.selectNext()).generateEntityConstruct(), world, null);
