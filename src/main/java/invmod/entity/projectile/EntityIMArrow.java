...
public class EntityIMArrow extends EntityTippedArrow {
	...
	public void onUpdate() {
		...
		if (blockState.getMaterial() != Material.AIR) {
			...
			if (blockState.getBlock() == Blocks.GLASS) {
				...
				// blockState.getBlock().setBlockBoundsBasedOnState(this.world,
				// this.getPosition());
				// AxisAlignedBB axisalignedbb = blockState.getSelectedBoundingBox(this.world,
				// this.getPosition());
		...
		if (this.inGround) {
			...
			if (rtr != null)
				vec3 = new Vec3d(rtr.hitVec.x, rtr.hitVec.y, rtr.hitVec.z);
			...
			if (rtr != null) {
				if (rtr.entityHit != null) {
					...
					DamageSource damagesource = DamageSource.causeArrowDamage(this,
							this.shootingEntity != null ? this.shootingEntity : this);

					if (this.isBurning() && !(rtr.entityHit instanceof EntityEnderman))
					...
					if (rtr.entityHit.attackEntityFrom(damagesource, k)) {
						...
						// this.playSound("random.bowhit", 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F +
						// 0.9F));
	...
	/*
	 * @Override protected ItemStack getArrowStack() { // TODO Auto-generated method
	 * stub return null; }
	 */
