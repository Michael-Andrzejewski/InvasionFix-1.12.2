...
public class EntityIMZombie extends EntityIMMob implements ICanDig {
	...
	private static final DataParameter<Boolean> META_CHANGED = EntityDataManager.createKey(EntityIMZombie.class,
			DataSerializers.BOOLEAN); // 29
	private static final DataParameter<Integer> FLAVOUR = EntityDataManager.createKey(EntityIMZombie.class,
			DataSerializers.VARINT); // 28
	private static final DataParameter<Boolean> IS_SWINGING = EntityDataManager.createKey(EntityIMZombie.class,
	...
	public void onUpdate() {
		...
		if ((!this.world.isRemote) && (this.flammability >= 20) && (this.isBurning()))
			this.doFireball();
	}
	...
	/*
	 * @Override public int getTier() { return this.tier < 3 ? 2 : 3; }
	 */
	...
	protected boolean chargeAttack(Entity entity) {
		...
		entity.addVelocity(-MathHelper.sin(this.rotationYaw * 3.141593F / 180.0F) * knockback * 0.5F, 0.4D,
				MathHelper.cos(this.rotationYaw * 3.141593F / 180.0F) * knockback * 0.5F);
	 ...
	 * this.rand.nextInt(3) == 0 ? "invmod:bigzombie1" : null; }
	 * 
		...
		for (int xOffset = -1; xOffset < 2; xOffset++)
			for (int yOffset = -1; yOffset < 2; yOffset++)
				for (int zOffset = -1; zOffset < 2; zOffset++) {
					if ((this.world.isAirBlock(pos.add(xOffset, yOffset, zOffset)) || (this.world
							.getBlockState(pos.add(xOffset, yOffset, zOffset)).getMaterial().getCanBurn()))) {
