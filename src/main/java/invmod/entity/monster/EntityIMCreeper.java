...
public class EntityIMCreeper extends EntityIMMob {
	...
	private static final DataParameter<Integer> CREEPER_STATE = EntityDataManager.createKey(EntityIMCreeper.class,
		...
		if (this.explosionDeath) {
			...
			if (state > 0) {
				if (this.commitToExplode) {
					this.getMoveHelper().setMoveTo(this.posX + invmod.util.Coords.offsetAdjX[this.explodeDirection],
							this.posY, this.posZ + invmod.util.Coords.offsetAdjZ[this.explodeDirection], 0.0D);
				}
...
//	}
	...
	public float getBlockPathCost(PathNode prevNode, PathNode node, IBlockAccess terrainMap) {
		...
		if ((block != Blocks.AIR) && (!block.isPassable(terrainMap, new BlockPos(node.pos)))
				&& (block != /* BlocksAndItems.blockNexus */ModBlocks.NEXUS_BLOCK)) {
