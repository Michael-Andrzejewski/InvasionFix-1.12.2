...
public class IMWaveSpawner implements ISpawnerAccess {
	...
	public void beginNextWave(Wave wave) throws WaveSpawnerException {
		...
		if (this.debugMode)
			ModLogger.logDebug("Defined mobs this wave: " + getTotalDefinedMobsThisWave());
	}
	...
	public boolean attemptSpawn(EntityConstruct mobConstruct, int minAngle, int maxAngle) {
		...
		for (int j = 0; j < spawnTries; j++) {
			...
			if (!this.spawnMode) {
				...
				if (this.debugMode) {
					ModLogger.logDebug("[Spawn] Time: " + this.currentWave.getTimeInWave() / 1000 + "  Type: " + mob.toString() + "  Coords: " + spawnPoint.getPos().toString() + "  θ" + spawnPoint.getAngle() + "  Specified: " + minAngle + "," + maxAngle);
				}
		...
		ModLogger.logWarn("Could not find valid spawn for '" + EntityList.getEntityString(mob) + "' after " + spawnTries + " tries");
		return false;
	}

	private void generateSpawnPoints() {
		if (this.nexus.getWorld() == null) return;
		...
		ModLogger.logDebug("Num. Spawn Points: " + Integer.toString(this.spawnPointContainer.getNumberOfSpawnPoints(SpawnType.HUMANOID)));
	}

	private void addValidSpawn(EntityIMLiving entity, List<SpawnPoint> spawnPoints, int x, int y, int z) {
		entity.setLocationAndAngles(x, y, z, 0.0F, 0.0F);
		if (entity.getCanSpawnHere()) {
			int angle = (int) (Math.atan2(this.nexus.getPos().getZ() - z, this.nexus.getPos().getX() - x) * 180.0D / Math.PI);
