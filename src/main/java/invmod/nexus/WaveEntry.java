...
public class WaveEntry
{
    ...
    if (this.toNextSpawn <= 0)
    {
      ...
      if (amountToSpawn > 0)
      {
        ...
        while (amountToSpawn > 0)
        {
          ...
          {
            ModLogger.logWarn("A selection pool in wave entry " + toString() + " returned empty");
  ...
  private void reviseSpawnAngles(ISpawnerAccess spawner)
  {
    ...
    {
      ModLogger.logInfo("Can't find a direction with enough spawn points: " + this.minPointsInRange + ". Lowering requirement.");

      this.minPointsInRange = 1;
    }
    else if (this.maxAngle - this.minAngle < 360)
    {
      ModLogger.logInfo("Can't find a direction with enough spawn points: " + this.minPointsInRange + ". Switching to 360 degree mode for this entry");
